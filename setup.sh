...
if [[ "$setup_tokens" =~ ^[Yy]$ ]]; then
    ...
    if [[ "$setup_notion" =~ ^[Yy]$ ]]; then
        ...
        echo ""
        echo "7. Click 'Submit'"
        echo "8. Copia el token completo"
...
EOF
    ...
    if [[ "$setup_miro" =~ ^[Yy]$ ]]; then
        ...
        echo "10. En 'Access token' o 'Developer access token', busca:"
        echo "    - Click 'Generate new token' si no hay"
        echo "    - Si ya existe, cópialo"
        echo "11. IMPORTANTE: Copia el 'Access token' (alfanumérico)"
        echo "    ✓ Correcto: 12a3b4c5d6e7f8..."
        echo "    ✗ Incorrecto: Token que empieza con 'ey' (ese es OAuth)"
        echo ""
        echo -e "${BOLD}⚠️ PASO CRÍTICO (sin esto NO funciona):${NC}"
        echo ""
