...
EOF
    ...
    if [[ "$setup_miro" =~ ^[Yy]$ ]]; then
        ...
        echo ""
        echo "9. Al fondo de la página, busca 'Install app and get OAuth token'"
        echo "10. Click en ese botón"
        echo "11. Selecciona tu Developer team"
        echo "12. Click 'Install & authorize'"
        echo "13. Copia el Access Token que aparece"
        echo "    (Es un JWT que empieza con 'eyJ' - esto es correcto)"
        echo ""
        echo -e "${BOLD}⚠️ PASO CRÍTICO (sin esto NO funciona):${NC}"
        echo ""
